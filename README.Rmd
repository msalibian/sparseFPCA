...
robust FPCA method introduced in [Boente and Salibian-Barrera, 2020](https://github.com/msalibian/sparseFPCA).
...
The `sparseFPCA` package implements the robust functional principal components analysis (FPCA)  estimator introduced in [Boente and Salibian-Barrera, 2020](https://github.com/msalibian/sparseFPCA). `sparseFPCA` computes 
robust estimators for the mean and covariance (scatter)
function, and the associated eigenfunctions. 
It can be used with functional
data sets where there only a few  observations available
per curve (possibly recorded at irregular intervals).


## An example - CD4 counts data

Below we illustrate the use of our method and compare it
