...
isSymmetric(ours.ls$cov.fun)
isSymmetric(ours.r$cov.fun)
isSymmetric(pace$smoothedCov)

round(eigen(ours.ls$cov.fun)$values[1:20] * .109, 3)
round(svd(ours.ls$cov.fun)$d[1:20] *.109, 3)

round(eigen(ours.r$cov.fun)$values[1:20]*.109, 3)
round(svd(ours.r$cov.fun)$d[1:20] *.109, 3)

m1 <- ours.ls$cov.fun
m2 <- pace$smoothedCov
m1.e <- eigen(m1)
tmp <- m1.e$values >= 0
tmp2 <- m1.e$vectors[, tmp]
m11 <- tmp2 %*% diag(m1.e$values[tmp]) %*% t(tmp2)
...
G.ls <- ours.ls$cov.fun
filled.contour(tt, ss, G.ls, main='LS')
filled.contour(tt, ss, m11, main='Pos LS')

m1.r <- ours.ls$cov.fun
m1.re <- eigen(m1.r)
tmp <- m1.re$values >= 0
tmp2 <- m1.re$vectors[, tmp]
m11.r <- tmp2 %*% diag(m1.re$values[tmp]) %*% t(tmp2)

ss <- tt <- ours.r$ss
G.r <- ours.r$cov.fun
filled.contour(tt, ss, G.r, main='ROB')
filled.contour(tt, ss, m11.r, main='Pos ROB')

m2 <- pace$smoothedCov
m2.e <- eigen(m2)
tmp <- m2.e$values >= 0
tmp2 <- m2.e$vectors[, tmp]
m22 <- tmp2 %*% diag(m2.e$values[tmp]) %*% t(tmp2)

ss <- tt <- pace$workGrid
G.pace <- pace$smoothedCov
filled.contour(tt, ss, G.pace, main='PACE')
filled.contour(tt, ss, m22, main='Pos PACE')
